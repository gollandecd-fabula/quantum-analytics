...
def _register_trusted_trace(
    ...
    entry = (deepcopy(dict(resolution)), _rules_fingerprint(rules))
...
def _matches_trusted_trace(
    ...
    return (
        ...
        and _rules_fingerprint(rules) == expected_rules_fingerprint
    )
...
def resolve_rule(
    ...
    resolution = _resolve_rule(rules, context)
    _register_trusted_trace(resolution, rules)
    return resolution
...
def _selected_rule(
    ...
    lookup = {
        ...
        for rule in _normalized_rules(rules)
    }
...
def evaluate_resolved_rule(
    ...
    if not _matches_trusted_trace(resolution, rules):
