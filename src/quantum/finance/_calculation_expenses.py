...
def calculate_other_expense(
    ...
    for item in raw:
        ...
        value = _normalize_value(
            _value_from_dict(
                item["value"],
                source_id="other_expense:" + str(component_id),
            ),
            policy,
        )
        if (
            value.value_type != "MONEY"
            or value.currency != currency
            or value.unit not in {"MONEY", "MONEY_PER_ITEM"}
        ):
            return _make_nonvalid(
                "BLOCKED",
                value_type="MONEY",
        ...
        if value.state == "VALID" and value.value < 0:
            raise FinanceError(
                "OTHER_EXPENSE_NEGATIVE_FORBIDDEN:" + str(component_id)
            )
        amount = value
        if value.unit == "MONEY_PER_ITEM":
            if net.state == "VALID" and net.value < 0:
                amount = _make_nonvalid(
                    ...
                    source_ids=net.source_ids + value.source_ids,
                )
            else:
                amount = _propagate(
                    [net, value],
