...
def reference_calculate(case:dict[str,Any])->dict[str,dict[str,str|None]]:
    ...
    net_units=int(case["gross_sales_units"])-int(case["returned_units"])
    ...
    marketplace=(inputs["gross_sales_amount"]-inputs["discounts_amount"]+inputs["subsidies_amount"]-inputs["marketplace_commission_amount"]-inputs["forward_logistics_amount"]-inputs["reverse_logistics_amount"]-inputs["storage_amount"]-inputs["advertising_amount"]-inputs["fines_withholdings_amount"])
