...
def scan_forbidden_markers(root: Path) -> None:
    ...
    for path in root.rglob("*"):
        ...
        is_test_contract = bool(relative.parts) and relative.parts[0] == "tests"
        if (
            not is_scanner
            and not is_test_contract
            and _WRITE_ENABLE_PATTERN.search(text)
        ):
            violations.append(
                f"{relative}: marketplace write enablement"
            )
    if violations:
