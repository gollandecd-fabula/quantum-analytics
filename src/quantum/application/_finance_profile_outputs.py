...
_ACTION_LABELS = {
    ...
    "RECOMMENDATION_BUILD_BLOCKED": (
        "Устранить причину блокировки рекомендаций"
    ),
}
...
def _recommendation_action_label(item: Mapping[str, Any]) -> str:
    ...
    return _ACTION_LABELS.get(code, "Проверить рекомендацию")
...
def _recommendation_rows(
    ...
    for record in recommendations:
        ...
        rows.append([
            ...
            ("D", _recommendation_action_label(item), False),
            (
                "E",
                current.get("amount", "")
    ...
    for error in recommendation_errors:
        rows.append([
            ...
            ("H", "LOW", False),
            ("I", error, False),
            ("J", "RECOMMENDATION_BUILD_BLOCKED", False),
        ])
    if len(rows) == 1:
        rows.append([
            ("A", "", False),
    ...
    *,
    ...
    for record in recommendations:
        ...
        recommendation_cards.append(
            ...
            f'<p><strong>{escape(_recommendation_action_label(item))}</strong></p>'
            f'<p class="muted">Код: {escape(str(item.get("action_code") or "—"))}</p>'
