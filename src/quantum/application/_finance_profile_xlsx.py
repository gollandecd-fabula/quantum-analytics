    ...
    *,
    ...
    for row_index, values in rows:
        ...
        for field_name in required_fields:
            aliases = _HEADER_ALIASES.get(
                field_name
            ) or _DETAILED_REQUIRED_HEADERS.get(field_name)
            if aliases is None:
...
def detect_products_from_xlsx(path: Path) -> tuple[ProductRecord, ...]:
    ...
    for row_index, values in rows:
        ...
        for preferred in _PRODUCT_ID_PRIORITY:
            ...
            if len(matches) > 1:
                raise FinanceProfileError(
                    "XLSX_HEADER_DUPLICATE:product_id"
                )
            if matches:
    ...
    def optional_position(field_name: str) -> int | None:
        ...
        if len(matches) > 1:
            raise FinanceProfileError(
                "XLSX_HEADER_DUPLICATE:" + field_name
            )
    ...
    for row_index, values in rows:
        if row_index <= header_index or not any(
            ...
            continue
        product_id = (
            values[product_position].strip()
            if product_position < len(values)
            else ""
        )
        if not product_id:
            continue
        ...
        if (
            ...
            and previous.detected_group != group_name
        ):
            raise FinanceProfileError(
