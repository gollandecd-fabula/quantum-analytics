...
class FinanceCenterPagesMixin:
        ...
        def _build_decision_page(self) -> None:
            ...
            ttk.Button(action_bar, text="Добавить отчёты", style="Success.TButton", command=self.add_reports).pack(side=tk.LEFT)
        ...
        def _build_reports_page(self) -> None:
            ...
            ttk.Button(controls, text="Добавить отчёты", style="Success.TButton", command=self.add_reports).pack(side=tk.LEFT)
            ttk.Button(controls, text="Повторить выбранный", command=self.repeat_selected).pack(side=tk.LEFT, padx=7)
