...
class FinanceCenterPagesMixin:
    ...
    def refresh_decision_center(self) -> None:
        ...
        selected_group = self.header_group_filter.get()
        if selected_group not in groups:
            selected_group = "Все товарные группы"
            self.header_group_filter.set(selected_group)
        all_profits = self._group_profit_rows()
        profits = (
            all_profits
            if selected_group == "Все товарные группы"
            else [row for row in all_profits if row[0] == selected_group]
        )
        ...
        if result is None:
            self._set_decision_card(
                ...
                cause="нет загруженных таблиц с финансовыми полями",
                action="загрузить любые доступные отчёты или архивы",
                effect="доступные показатели будут рассчитаны",
                confidence="высокая",
            )
        elif result.status == "CALCULATED_PARTIAL":
            self._set_decision_card(
                "problem",
                title="Часть показателей рассчитана",
                impact="доступные цифры сохранены; итог ограничен охватом",
                cause=(
                    result.missing_inputs[0]
                    if result.missing_inputs
                    else "частичный охват источников"
                ),
                action="добавить только перечисленные недостающие данные",
                effect="увеличится число рассчитанных показателей",
                confidence="высокая для рассчитанных цифр",
            )
        elif result.status != "CALCULATED":
            self._set_decision_card(
                "problem",
                title="Финансовые показатели пока не рассчитаны",
                impact="нет достаточного набора зависимостей",
                cause=(result.missing_inputs[0] if result.missing_inputs else "неполные данные"),
                action="добавить перечисленные данные",
                effect="станут доступны зависимые показатели",
                confidence="высокая",
            )
        elif negative:
        ...
        if selected_group != "Все товарные группы":
            alert_lines.append(
                "• Фильтр группы применяется к решениям и списку лидеров; "
                "KPI остаются итогом периода, потому что налог не распределён "
                "по товарным группам без подтверждённой политики."
            )
            self._set_text(self.alerts_text, "\n".join(alert_lines))
    ...
    def refresh_domain_pages(self) -> None:
        ...
        if not self.profile.groups:
            product_lines.append(
                "Товары пока не определены. Загрузите любую таблицу с явным "
                "артикулом/SKU или настройте данные после импорта."
            )
        ...
        self._set_text(
            self.supply_text,
            "Модуль не подменяет отсутствие данных. Для прогноза остатков "
            "подойдёт любой загруженный файл или архив, где явно присутствуют "
            "остатки, поставки и даты. Пока таких полей нет, финансовые расчёты "
            "продолжаются, а прогноз поставки отмечен как недоступный.",
        )
        self._set_text(
