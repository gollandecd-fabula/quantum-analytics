...
def _normalized_sensitive_token(value: str) -> str:
...
class XlsxSchemaExpectation:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            not isinstance(self.header_sha256, str)
            or _HEX_SHA256.fullmatch(self.header_sha256) is None
        ):
    ...
    @property
    def match_profile(self) -> tuple[object, ...]:
        return (
            self.package_kind,
            self.sheet_name,
            self.sheet_count,
...
class XlsxInspectionPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        match_profiles = {item.match_profile for item in self.schemas}
        if len(match_profiles) != len(self.schemas):
            raise XlsxInspectionError("XLSX_SCHEMA_PROFILE_AMBIGUOUS")
        ...
        for token in self.prohibited_header_tokens:
            safe = _safe_text(token, "XLSX_PROHIBITED_TOKEN_INVALID")
            normalized = _normalized_sensitive_token(safe)
            if not normalized:
                raise XlsxInspectionError("XLSX_PROHIBITED_TOKEN_INVALID")
