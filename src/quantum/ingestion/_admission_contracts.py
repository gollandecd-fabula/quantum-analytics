...
class DatasetDeclaration:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.reporting_period_start, date) or not isinstance(self.reporting_period_end, date):
        ...
        if not isinstance(self.timezone, str) or not self.timezone:
            raise AdmissionError("DATASET_TIMEZONE_INVALID")
        try:
            ZoneInfo(self.timezone)
        except ZoneInfoNotFoundError as exc:
            raise AdmissionError("DATASET_TIMEZONE_INVALID") from exc
        ...
        if self.control_totals_sha256 is not None:
            _sha(self.control_totals_sha256, "DATASET_CONTROL_TOTALS_HASH_INVALID")
        if not isinstance(self.data_categories, tuple) or not self.data_categories:
            raise AdmissionError("DATASET_CATEGORIES_REQUIRED")
        categories = tuple(_safe_identifier(item, "DATASET_CATEGORY_INVALID") for item in self.data_categories)
        ...
        _safe_identifier(self.owner_authority_reference, "DATASET_AUTHORITY_REFERENCE_INVALID")
        if self.lawful_authority_attested is not True:
            raise AdmissionError("DATASET_AUTHORITY_ATTESTATION_REQUIRED")
        retention = _aware_utc(self.retention_deadline, "DATASET_RETENTION_TIMEZONE_REQUIRED")
        declared = _aware_utc(self.declared_at, "DATASET_DECLARED_TIMEZONE_REQUIRED")
...
class StorageControlEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        _sha(self.original_file_sha256, "STORAGE_EVIDENCE_FILE_HASH_INVALID")
        _sha(self.storage_key_sha256, "STORAGE_EVIDENCE_KEY_HASH_INVALID")
        _safe_identifier(self.verifier_account_id, "STORAGE_EVIDENCE_VERIFIER_INVALID")
...
class AdmissionDecision:
    ...
    def __post_init__(self) -> None:
        ...
        _safe_identifier(self.actor_account_id, "DATASET_DECISION_ACTOR_INVALID")
