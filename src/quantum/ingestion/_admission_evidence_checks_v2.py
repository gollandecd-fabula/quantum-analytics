...
def require_evidence_binding(record, dataset_evidence, storage_evidence):
    ...
    if _uuid(dataset_evidence.dataset_id, "DATASET_EVIDENCE_DATASET_INVALID") != declaration.dataset_id:
    ...
    if (dataset_evidence.reporting_period_start != declaration.reporting_period_start or dataset_evidence.reporting_period_end != declaration.reporting_period_end or dataset_evidence.timezone != declaration.timezone):
    ...
    if dataset_evidence.structural_fingerprint_sha256 != inspection.structural_fingerprint_sha256:
        raise AdmissionError("DATASET_EVIDENCE_FINGERPRINT_MISMATCH")
    if (dataset_evidence.matched_schema_id != inspection.matched_schema_id or dataset_evidence.matched_schema_version != inspection.matched_schema_version or dataset_evidence.matched_schema_authority_reference != inspection.matched_schema_authority_reference):
        raise AdmissionError("DATASET_EVIDENCE_SCHEMA_MISMATCH")
    if _uuid(storage_evidence.dataset_id, "STORAGE_EVIDENCE_DATASET_INVALID") != declaration.dataset_id:
...
def require_evidence_times(record, dataset_evidence, storage_evidence, admitted_at):
    validation_time = record.latest_decision.decided_at
    dataset_verified = _aware_utc(dataset_evidence.verified_at, "DATASET_EVIDENCE_TIMEZONE_REQUIRED")
    storage_verified = _aware_utc(storage_evidence.verified_at, "STORAGE_EVIDENCE_TIMEZONE_REQUIRED")
