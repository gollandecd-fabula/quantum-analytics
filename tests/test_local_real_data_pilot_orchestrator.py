...
class LocalRealDataPilotOrchestratorTests(unittest.TestCase):
    ...
    def calculated_snapshot(self, results):
        return finance_result_snapshot(
            dataset_id=self.declaration.dataset_id,
            original_file_sha256=self.declaration.original_file_sha256,
            row_count=self.declaration.expected_row_count,
            finance_results=results,
            metric_bindings={"net_profit_amount": ("synthetic", "net_profit_amount")},
        )
    ...
    def execute(self, **overrides):
        values = {
            ...
            "storage_evidence_builder": lambda record: evidence(self.tenant, record),
            "observed_at": NOW + timedelta(minutes=1),
            "admitted_at": NOW + timedelta(minutes=2),
            "finance_requests": {"synthetic": self.request},
            "source_snapshot": self.source_snapshot(),
            "calculated_snapshot_builder": self.calculated_snapshot,
    ...
    def test_non_loopback_scope_fails_before_admission(self) -> None:
        ...
        with self.assertRaisesRegex(LocalPilotExecutionError, "PILOT_LOOPBACK_REQUIRED"):
    ...
    def test_snapshot_binding_requires_existing_finance_metric(self) -> None:
        with self.assertRaisesRegex(
            ...
            finance_result_snapshot(
                ...
                metric_bindings={"net_profit_amount": ("synthetic", "missing")},
            )
