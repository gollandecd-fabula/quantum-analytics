...
def rewrite_xlsx_part(workbook: bytes, part_name: str, transform) -> bytes:
    output = BytesIO()
    with ZipFile(BytesIO(workbook)) as current, ZipFile(output, "w", compression=ZIP_DEFLATED) as rewritten:
...
def policy(*, headers: tuple[str, ...] = HEADERS, formulas: int = 0):
    ...
    return XlsxInspectionPolicy(
        ...
        prohibited_header_tokens=("phone", "email", "address", "customer name", "телефон", "адрес", "фио"),
    )
...
def evidence(tenant: TenantContext, dataset, **overrides):
    values = {
        ...
        "verifier_account_id": tenant.account_id + "-storage-reviewer",
    }
...
def dataset_evidence(tenant: TenantContext, dataset, **overrides):
    ...
    values = {
        ...
        "dataset_id": dataset.declaration.dataset_id,
        "original_file_sha256": dataset.declaration.original_file_sha256,
        "owner_authority_reference": dataset.declaration.owner_authority_reference,
        "reporting_period_start": dataset.declaration.reporting_period_start,
        "reporting_period_end": dataset.declaration.reporting_period_end,
        "timezone": dataset.declaration.timezone,
        "control_totals_sha256": dataset.declaration.control_totals_sha256,
        "policy_content_hash": dataset.policy_content_hash or "0" * 64,
        "workbook_sha256": inspection.workbook_sha256 if inspection else "0" * 64,
        "structural_fingerprint_sha256": inspection.structural_fingerprint_sha256 if inspection else "0" * 64,
        "matched_schema_id": inspection.matched_schema_id if inspection and inspection.matched_schema_id else "unmatched",
        "matched_schema_version": inspection.matched_schema_version if inspection and inspection.matched_schema_version else "unmatched",
        "matched_schema_authority_reference": inspection.matched_schema_authority_reference if inspection and inspection.matched_schema_authority_reference else "unmatched",
        ...
        "verified_at": NOW + timedelta(minutes=1),
        "verifier_account_id": tenant.account_id + "-dataset-reviewer",
    }
    values.update(overrides)
